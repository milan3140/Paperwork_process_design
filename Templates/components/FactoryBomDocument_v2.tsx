 ...
 *  Renders paginated doc-page divs (Letter size) via PaginatedDocument with:
 *  - Title row, metadata table header, part rows, summary, DFM notes, manufacturing notes.
 *
 *  PaginatedDocument handles page distribution, headers, and footers automatically. */
export const FactoryBomDocument = React.forwardRef<HTMLDivElement, FactoryBomDocumentProps>(
  function FactoryBomDocument({ data }, ref) {
          ...
          <>
            <div className="flex gap-[var(--sp-4)]">
              ...
              <div className="flex-1 flex items-center justify-between">
                ...
                {/* issueDate shown inline since header no longer has custom date display */}
            ...
            <div className="text-[length:12px] font-bold text-[color:var(--color-error)] mt-[var(--sp-1)]">
    ...
    /* ── Main table with metadata header, column headers, all part rows, and summary ── */
    sections.push({
      key: 'table',
      content: (
        <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            ...
            <tr className="bg-[var(--gray-50)]">
              <td className={`${TH} text-center border-l border-l-[var(--gray-100)] border-r border-r-[var(--gray-100)]`}>尺寸圖紙</td>
              ...
              <td className={TH} style={{ padding: 0 }}>
                {/* Nested table for 報價 sub-columns */}
                <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
                  <colgroup>
          ...
          <tbody>
            {data.parts.map((part, i) => (
              <FactoryBomRow
                key={`${part.partId}-${i}`}
                part={part}
                isLast={i === data.parts.length - 1}
              />
            ))}
            {/* Summary rows */}
            <FactoryBomSummary tierTotals={tierTotals} />
          </tbody>
        </table>
      ),
    });

    /* ── DFM notes: blank hand-writable area ── */
    sections.push({
      key: 'dfm-notes',
      content: <DfmNotesSection lineCount={data.dfmLineCount} />,
    });

    /* ── Manufacturing notes (注意事項) ── */
    const noteItems = data.notes ?? DEFAULT_NOTES;
