/**
 * PackingSlipDocument v13 — v12 with compound ID + filename in Geist Sans (was monospace).
 *
 * Change (vs v12):
 *   - Line-item card header: compound ID (e.g. "U26033148F_P01") and filename
 *     (e.g. "115425AT_P064454846468483.STEP") inherit the document's Geist Sans
 *     font instead of `monospace`. Reads as editorial typography, pairs better
 *     with the Geist body text below.
 *
 * Trade-off: alphanumeric codes lose column alignment (proportional widths),
 * but gain visual cohesion with the rest of the doc.
 *
 * All other v12 properties preserved. Original v12 description below.
 *
 *
 * PackingSlipDocument v12 — v11 with line-item cards fully de-tinted + reinforced borders.
 *
 * Line-item changes (vs v11):
 *   - Card header band (compound ID + filename): no background, was #FAFAFA
 *   - Card footer (Required Document Checklist): no background, was #FAFAFA
 *   - Outer card border: gray-200 → gray-300 (#B8B8B8) — noticeably stronger frame
 *   - Inner separators (header↓body, image↓dims, body↓checklist, etc.): gray-150 → gray-200
 *     — zone dividers carry visual weight now that tint is gone.
 *
 * Rationale: pure wireframe/skeleton look for line items. White interior, crisp
 * outline. Background-tint removed → contrast shifted entirely to borders.
 *
 * All other v11 properties preserved (pure-black palette, parties-first header,
 * Page X/Y in meta grid, Gross WT column, etc.). Original v11 description below.
 *
 *
 * PackingSlipDocument v11 — v10 + layout reorder per Q3/Q4 recommendations.
 *
 * Q3: parties block (Sold To / Ship To) moves BEFORE carrier block in the
 *     header section — recipients are the document's primary identifier.
 * Q4: "Page X of Y" relocated from the title row into the right-side meta
 *     grid (above Date), keeping the title clean and treating page count as
 *     metadata, consistent with Order/PO Ref placement.
 *
 * Inherits all v10 properties (all-black body, 4 protected gray labels,
 * GROSS WT. column, etc.). Original v10 description below.
 *
 * v10 — v9 + all body text in solid black.
 *
 * Only these labels stay gray (CARD_LABEL):
 *   Qty Ordered · Qty Shipped · Required Document Checklist · Note
 * All other previously-gray text (gray-400/500/600/700/800) is gray-900.
 *
 * Original v9 description preserved below.
 *
 * v9 — v7 with brand primary (InstaVoxel purple #2E0D77) header
 *
 * v7 changes from v5:
 *   - Compressed card: thumbnail 210×160 → 180×110, header padding 8→5px, qty/docs padding 10→6px
 *   - Enabled `tightPagination` on PaginatedDocument: pagination bin-packing uses
 *     the actual render `gap` (3) instead of MIN_GAP (12), which can save ~9px
 *     per section gap. Often lets a nearly-overflowing doc fit in 1 page and
 *     auto-suppresses "see next page" continuation hints.
 *
 * v5 changes from v4:
...
const INLINE_LABEL: React.CSSProperties = {
  fontSize: 11,           /* Sold To / Ship To label — 11px */
  fontWeight: 400,
  /* v10: was gray-400 → black. CARD_LABEL is the only label that stays gray. */
  color: 'var(--gray-900)',
...
/** Compute gross shipment weight (lb) for a line item: unitWeight × qty. */
function computeGrossWeightLb(unitWeight: string, qty: number): string {
  const match = unitWeight.match(/([\d.]+)\s*kg/i);
  if (!match) return '';
  return (parseFloat(match[1]) * 2.20462 * qty).toFixed(2) + ' lb';
}
  ...
  return (
    <div style={{ display: 'flex', gap: 'var(--sp-3)', alignItems: 'flex-start' }}>
      ...
      <div>
        ...
        <div className="text-[color:var(--gray-900)] leading-[1.5]" style={{ fontSize: 14 }}>
 ...
 * Used for carrier info (left or right column). v9: 11px label + value, no bold.
 */
function InfoGrid({ items }: { items: { label: string; value: string }[] }) {
  ...
  return (
    ...
    >
      {items.map((item, i) => (
        <React.Fragment key={i}>
          <span className="text-[color:var(--gray-900)] uppercase tracking-[var(--doc-tracking-label)]" style={{ fontSize: 10, lineHeight: 1, fontWeight: 400 }}>
            {item.label}
          </span>
          <span className="text-[color:var(--gray-900)]" style={{ fontSize: 11, lineHeight: 1, fontWeight: 400 }}>
            {item.value}
          </span>
        </React.Fragment>
      ))}
...
function PartCard({ item, orderId, defaultDocuments }: { item: PackingSlipItem; orderId: string; defaultDocuments?: string[] }) {
  ...
  return (
    <div style={{
      ...
      {/* ── Card header: compound ID + filename ──
       *   Grid layout so filename's left edge aligns with the right column body
       *   (Qty Ordered). Left cell width = thumbnail column width (140px) minus
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{
          ...
          {/* Dims + weight below thumbnail, centered */}
          <div style={{
            ...
            <div style={{ fontSize: 'var(--doc-text-body)', fontWeight: 400, color: 'var(--gray-900)', whiteSpace: 'nowrap' }}>
              {convertDimsToInch(item.dims)}{' · '}{convertWeightToLb(item.unitWeight)}
            </div>
            <div style={{ fontSize: 'var(--doc-text-body)', fontWeight: 300, color: 'var(--gray-900)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ ...CARD_LABEL, fontSize: 11 }}>Qty Ordered</div>
              <div style={{
                fontSize: 17,
                fontWeight: 400,
                color: 'var(--gray-900)',
            ...
            <div>
              <div style={{ ...CARD_LABEL, fontSize: 11 }}>Qty Shipped</div>
              <div style={{
                fontSize: 17,
                fontWeight: 400,
                color: qtyColor,
            ...
            <div>
              <div style={{ ...CARD_LABEL, fontSize: 11 }}>Gross WT.</div>
              <div style={{
                fontSize: 17,
                fontWeight: 400,
                color: 'var(--gray-900)',
                ...
                {computeGrossWeightLb(item.unitWeight, item.qtyShipped)}
              </div>
            </div>
          </div>
          ...
          {(() => {
            ...
            return (
              <div style={{
                ...
                <div style={{ ...CARD_LABEL, fontSize: 11, marginBottom: 'var(--doc-sp-half)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {docs.map((name, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                        ...
                        <span style={{ fontSize: 16, color: 'var(--gray-900)' }}>
                          {name}
                        </span>
                      </div>
                    ))}
          ...
          <div style={{
            ...
            {item.note && (
              <span style={{ fontSize: 14, color: 'var(--gray-900)' }}>{item.note}</span>
            )}
          </div>
...
export const PackingSlipDocumentV13 = React.forwardRef<HTMLDivElement, PackingSlipDocumentProps>(
  function PackingSlipDocumentV13({ data }, ref) {
    ...
    sections.push({
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          {/* Carrier info — v11: moved AFTER addresses (operational details) */}
          <div
    ...
    sections.push({
      ...
      group: 'items',
      content: (
        <div data-el="PackingSlipDocument-items" className="pt-[var(--doc-sp-1-5)] border-t border-[var(--gray-150)]">
          <SectionLabel className="!text-[color:var(--gray-900)] !text-[10px] !font-normal !border-b-0 !pb-0">
    ...
    sections.push({
      ...
      content: (
        ...
        >
          <span className="text-[color:var(--gray-900)]" style={{ fontSize: 13 }}>
            {data.items.length} line item{data.items.length !== 1 ? 's' : ''}
          </span>
          <span className="text-[color:var(--gray-900)]" style={{ fontSize: 13 }}>
            Ordered:{' '}
            <span className="text-[color:var(--gray-900)]" style={{ fontSize: 16, fontWeight: 400 }}>{totalOrdered}</span> units
          </span>
          <span
            style={{ fontSize: 13, color: 'var(--gray-900)' }}
          >
            Shipped:{' '}
            <span style={{ color: 'var(--gray-900)', fontSize: 16, fontWeight: 400 }}>
              {totalShipped}
            </span>{' '}units
            {hasShortage && (
      ...
      sections.push({
        ...
        content: (
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--gray-900)', marginTop: 'var(--doc-sp-1-5)', lineHeight: 1.6 }}>
    ...
    return (
      <div ref={ref} data-comp="PackingSlipDocument" data-variant="v11">
        <PaginatedDocument
            ...
            <div data-el="PackingSlipDocument-titleRow" className="flex items-start justify-between" style={{ marginBottom: 8 }}>
              <div>
                ...
                <div
                  ...
                  style={{ fontSize: 28 }}
                >
                  <span className="font-bold text-[color:var(--color-primary)]">Packing Slip</span>
                </div>
                <div className="flex items-center gap-[var(--sp-3)] mt-[var(--doc-sp-half)]">
                  <span className="font-light text-[color:var(--gray-900)] tracking-[var(--doc-tracking-title)]" style={{ fontSize: 22 }}>
                    #{displaySlipId}
                  </span>
                  {data.isPartial && data.shipmentLabel && (
                    <span className="uppercase tracking-[var(--doc-tracking-label)] px-[var(--sp-2)] py-[var(--doc-sp-half)] bg-[var(--color-primary)] text-white rounded-[var(--radius-sm)]" style={{ fontSize: 10, fontWeight: 400 }}>
                      {data.shipmentLabel}
                    </span>
                  )}
                </div>
              ...
              >
                ...
                {metaItems.map((item, i) => (
                  <React.Fragment key={i}>
                    <span
                      className="text-[color:var(--gray-900)] uppercase tracking-[var(--doc-tracking-label)] text-right"
                      style={{ fontSize: 10, lineHeight: 1, fontWeight: 400 }}
                    >
                      {item.label}
                    </span>
                    <span
                      className={[
                        'text-right',
                        item.highlight ? 'text-[color:var(--color-primary)]' : 'text-[color:var(--gray-900)]',
                      ].join(' ')}
                      style={{ fontSize: 11, lineHeight: 1, fontWeight: 400 }}
                    >
                      {item.value}
                    </span>
                  </React.Fragment>
...
export default PackingSlipDocumentV13;
