...
export interface Scenario {
  ...
  unitPrice: number;
  leadTimeDays: number;
  /** Cost basis for price calculation */
  cost?: number;
  /** Margin percentage (unitPrice = cost * (1 + marginPercent/100)) */
  marginPercent?: number;
  /** Manual override of the calculated unit price */
...
export function computeUnitPrice(s: Scenario): number {
  if (s.priceOverride != null) return s.priceOverride;
  if (s.cost != null && s.marginPercent != null) return Math.round(s.cost * (1 + s.marginPercent / 100) * 100) / 100;
...
/** Built-in dimensions that can be toggled for comparison */
export type BuiltinCompareDimension = 'qty' | 'leadTime' | 'location' | 'material' | 'finish';
/** Dimensions that can be toggled on/off per Part for comparison (built-in + custom:{id}) */
export type CompareDimension = BuiltinCompareDimension | `custom:${string}`;

/** Custom dimension definition (per Part) */
...
export interface QuotePart {
  ...
  /** Optional per-part note (shown in PDF below the part's pricing) */
  note?: string;
  /** Custom dimensions defined for this Part */
...
export interface QuoteBuilderData {
  ...
  extraNotes: string[];
  /** v3: Quote-level notes (shown in PDF before T&C) */
  quoteNotes?: string;
...
/** Which fields vary across scenarios within a Part (built-in + custom:{id}) */
export type VaryingDimension = 'qty' | 'location' | 'material' | 'finish' | 'leadTime' | `custom:${string}`;
