...
function PriceCell({ scenarios, annotations, showLeadTime = true }: {
  ...
  return (
    <td className={`${TD} text-right`}>
      {scenarios.map((s, i) => (
        <div key={s.id} className={i > 0 ? 'mt-[var(--sp-2,8px)] pt-[var(--sp-2,8px)] border-t border-dashed border-[var(--gray-200,#e2e0e8)]' : ''}>
...
function PartHeader({ part, analysis }: { part: QuotePart; analysis: DimensionAnalysis }) {
  ...
  return (
    <div className="mb-[var(--sp-1,4px)]">
      ...
      {details.length > 0 && (
        <span className="text-[length:var(--doc-text-secondary,9px)] text-[color:var(--gray-400,#9a96a8)] ml-[var(--sp-2,8px)]">
          {details.join(' · ')}
        </span>
      )}
    </div>
  ...
  return (
    <div>
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {rowValues.map((rv, ri) => (
            <tr key={ri} className={ri < rowValues.length - 1 ? 'border-b border-[var(--gray-150,#eae8ef)]' : ''}>
    ...
    <div>
      ...
      <div className="flex flex-col gap-[var(--sp-3,12px)]">
        {groupValues.map((gv, gi) => {
          ...
          return (
            <div key={gi} className="border border-[var(--gray-150,#eae8ef)] rounded-[var(--radius-sm,4px)] p-[var(--sp-2,8px)]">
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {part.scenarios.map((s, i) => {
            ...
            return (
              <tr key={i} className={i < part.scenarios.length - 1 ? 'border-b border-[var(--gray-150,#eae8ef)]' : ''}>
