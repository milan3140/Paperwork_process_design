...
function validatePart(part: QuotePart): ValidationError[] {
  ...
  if (part.scenarios.length >= 2) {
    ...
    function buildFingerprint(s: Scenario): string {
      ...
      // Label always participates (it's the escape hatch for distinguishing otherwise-identical options)
      parts.push(`c:${s.customLabel?.trim() || ''}`);
      return parts.join('|');
    }
      ...
      if (!allSamePrice) {
        for (const idx of indices) {
          ...
          const hint = enabled.length === 0
            ? 'Enable a Compare dimension or add a Label to distinguish'
            : 'Add a Label to distinguish';
          errors.push({
            path: `scenario:${s.id}`,
            field: '_collision',
