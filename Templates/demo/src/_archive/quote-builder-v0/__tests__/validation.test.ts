...
function mkQuote(parts: Partial<QuotePart>[]): QuoteBuilderData {
  return {
    ...
    date: 'March 25, 2026',
    validDays: 30,
    customer: { companyName: 'Acme', contactName: 'John' },
...
describe('scenario validation', () => {
  ...
  it('leadTimeDays = 1.5 (non-integer) → error', () => {
    const result = validateQuote(mkQuote([{
...
describe('cross-scenario validation', () => {
  ...
  it('same qty+price but different location → NOT duplicate, no warning', () => {
    const result = validateQuote(mkQuote([{
      scenarios: [
        mkScenario({ qty: 10, unitPrice: 100, location: 'TW' }),
  ...
  it('BUG FIX: different leadTime → NOT collision even with same qty/location/price', () => {
    // User scenario: Option 2 (QTY 2, $20, 20 days) and Option 3 (QTY 2, $30, 21 days)
    const result = validateQuote(mkQuote([{
      scenarios: [
