...
function mkQuote(parts: QuotePart[], overrides?: Partial<QuoteBuilderData>): QuoteBuilderData {
  return {
    quoteId: 'Q2603251A',
    date: 'March 25, 2026',
    validDays: 30,
...
describe('fixed sections match real email text', () => {
  ...
  it('has Quote ID header', () => {
    expect(email).toContain('========== Quote ID: Q2603251A | Date: March 25, 2026 ==========');
  });
  ...
  it('has Shipping section', () => {
    ...
    expect(email).toContain('not included in the above pricing');
    expect(email).toContain('preferred carrier account');
  });

  it('has Payment Terms', () => {
    expect(email).toContain('Payment Terms');
    expect(email).toContain('U.S. dollars');
    expect(email).toContain('full upfront payment');
    expect(email).toContain('Wire');
    expect(email).toContain('Credit Card Online Invoice (3% processing fee)');
    expect(email).toContain('ACH (U.S. domestic client only)');
  });
