/**
 * PackingSlipDemo v12 — v11 with line-item cards fully de-tinted + reinforced borders.
 *   - No background on card header band or checklist footer.
 *   - Outer card border darkened (gray-200 → gray-300).
 *   - Inner separators darkened (gray-150 → gray-200).
 *
 * Original v11 description below.
 *
 * PackingSlipDemo v11 — v10 + Q3/Q4 layout reorder:
 *   Q3: Sold To / Ship To moved BEFORE carrier in header section
 *   Q4: Page X of Y moved from title row into right-side meta grid (above Date)
 *
 * Original v10 description below.
 *
 * v10 — v9 + all body text in solid black except 4 protected labels:
 * Qty Ordered · Qty Shipped · Required Document Checklist · Note (stay gray).
 *
 * Original v9 description preserved below:
 * v9 — v7 with brand primary (InstaVoxel purple) header:
 *
...
import { PackingSlipDocumentV12 as PackingSlipDocument, type PackingSlipData } from '../../../components/PackingSlipDocument_v12';
import { DownloadPdfButton } from '../DownloadPdfButton';
import { MODEL_SHOT_1 as thumb1, MODEL_SHOT_2 as thumb2 } from '../../../components/_assets';

const THUMBS = [thumb1, thumb2];

function withThumbs(data: PackingSlipData, startIdx = 0): PackingSlipData {
  return {
    ...data,
    items: data.items.map((it, i) => ({
      ...it,
      thumbnail: THUMBS[(i + startIdx) % THUMBS.length],
    })),
  };
}
 ...
 * ════════════════════════════════════════════════════════════════ */
export default function PackingSlipDemoV11() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'var(--sp-10) 0', gap: 'var(--sp-4)' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div className="text-[length:var(--text-xs)] text-[color:var(--gray-400)] mt-1">
          v9: InstaVoxel brand primary header (#2E0D77 purple)
        </div>
      </div>

      {/* All three variants in one ref — prints together */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-8)' }}>
        <PackingSlipDocument data={withThumbs(sampleInternational, 0)} />
        <PackingSlipDocument data={withThumbs(sampleDomestic, 1)} />
        <PackingSlipDocument data={withThumbs(samplePartial, 0)} />
      </div>
