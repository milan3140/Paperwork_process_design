/**
 * SummaryDemo — Renders SummaryDocument with sample data + DownloadPdfButton
 *
 * Rewritten from the legacy `<iframe srcDoc={summaryHtml}>` pattern to the
 * standard React document pipeline. Pagination: single page (no split needed).
 * PDF export goes through DownloadPdfButton → buildHtmlFromDom() →
...
export default function SummaryDemo() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'var(--sp-10) 0', gap: 'var(--sp-4)' }}>
      <DownloadPdfButton filename="Summary" />
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div className="text-[length:var(--text-xs)] text-[color:var(--gray-400)] mt-1">
          {sampleSummary.quoteId} · Order Brief (monochrome · Geist Sans)
        </div>
      </div>

      <SummaryDocument data={sampleSummary} />
    </div>
  );
}
